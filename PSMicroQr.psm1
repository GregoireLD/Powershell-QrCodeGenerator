...
class MuEcc{
	...
	# The QR Code can tolerate about  7% erroneous codewords ->      L   =     1           0
	# The QR Code can tolerate about 15% erroneous codewords ->      M   =     0           1
	# The QR Code can tolerate about 25% erroneous codewords ->      Q   =     3           2

	hidden [MuEccEnum] $internalEcl
    ...
    [MuEcc] getHigherEcc(){
        [MuEcc] $tmpEcl = New-Object 'MuEcc' 1

        switch ($this.internalEcl) {
            "L" { $tmpEcl.setEccValue(0) }
            "M" { $tmpEcl.setEccValue(3) }
            Default {
                throw "getHigherEcc was called on a already Q Ecc"
            }
        }
        return $tmpEcl
    }
    ...
    [MuEcc] getLowerEcc(){
        ...
        switch ($this.internalEcl) {
            "Q" { $tmpEcl.setEccValue(0) }
            "M" { $tmpEcl.setEccValue(1) }
            Default {
                throw "getLowerEcc was called on a already L Ecc"
            }
        }
        return $tmpEcl
    }
    ...
    [bool] isMax(){
        if ($this.internalEcl -eq [MuEccEnum]::Q) {
            return $true
        }
        return $false
    }
    ...
    [bool] isMin(){
        if ($this.internalEcl -eq [MuEccEnum]::L) {
            return $true
        }
        return $false
	}
	...
	MuEcc([string] $val){
		...
		switch ($val) {
            "L" { $this.internalEcl = "L" }
            "Q" { $this.internalEcl = "Q" }
			"M" { $this.internalEcl = "M" }
            Default {
	...
	static [MuEcc] L()      {return New-Object 'MuEcc' 1}
	static [MuEcc] M()   {return New-Object 'MuEcc' 0}
	static [MuEcc] Q() {return New-Object 'MuEcc' 3}
}
...
class MuQrMode{
	...
	hidden MuQrMode([int] $qrmode, [int[]] $ccbits)
	{
		$this.modeBits = $qrmode
		$this.numBitsCharCount = $ccbits
	}
	
	MuQrMode([string] $qrmode){
	...
	static [MuQrMode] NUMERIC()      {return New-Object 'MuQrMode' 0x1,@( 10, 12, 14)}
	static [MuQrMode] ALPHANUMERIC() {return New-Object 'MuQrMode' 0x2,@(  9, 11, 13)}
	static [MuQrMode] BYTE()         {return New-Object 'MuQrMode' 0x4,@(  8, 16, 16)}
	static [MuQrMode] KANJI()        {return New-Object 'MuQrMode' 0x8,@(  8, 10, 12)}
...
class MuQrSegment {
	...
	static [MuQrSegment] makeBytes([byte[]] $data)
	{
		if (-not $data)
        {
		...
		return (New-Object 'MuQrSegment' ([MuQrMode]::BYTE()),$data.Length,$bb )
	}
	...
	static [MuQrSegment] makeNumeric([String] $digits)
	{
		if (-not $digits)
        {
		...
		return (New-Object 'MuQrSegment' ([MuQrMode]::NUMERIC()), $digits.length , $bb)
	}
	...
	static [MuQrSegment] makeAlphanumeric([String] $text)
	{
		if (-not $text)
        {
		...
		return (New-Object 'MuQrSegment' ([MuQrMode]::ALPHANUMERIC()), $text.length , $bb)
	}
	...
	static [MuQrSegment[]] makeSegments([String] $text)
	{
		if (-not $text)
        {
		...
		{
			$result += ([MuQrSegment]::makeNumeric($text))
		}
		elseif ($text -cmatch [QrCodeGlobal]::ALPHANUMERIC_REGEX )
		{
			$result += ([MuQrSegment]::makeAlphanumeric($text))
		}
		else
		{
			$enc = [system.Text.Encoding]::UTF8
			$bytes = $enc.GetBytes($text)
			$result += ([MuQrSegment]::makeBytes($bytes))
		}
		return $result
	}
	...
	MuQrSegment([MuQrMode] $qrmd, [int] $numCh, [QrBitBuffer] $data)
	{
		if (-not $qrmd)
        {
	...
	static [int] getTotalBits([MuQrSegment[]] $segs, [int] $version)
	{
		if (-not $segs)
		{
...
class MuQrCode {
    ...
    static [MuQrCode] encodeText([string] $text, [MuEcc] $ecl,[int] $mask=-1, [boolean] $boostEcl=$true)
    {
        if (-not $text)
        {
		...
		[MuQrSegment[]] $segs = [MuQrSegment]::makeSegments($text)
        return [MuQrCode]::encodeSegments($segs, $ecl,$mask,$boostEcl)
	}
    ...
    # largest version QR Code at the ECL, which means it is too long
    static [MuQrCode] encodeBinary([byte[]] $data, [MuEcc] $ecl,[int] $mask=-1, [boolean] $boostEcl=$true)
    {
		if (-not $data)
        {
		...
		[MuQrSegment] $seg = [MuQrSegment]::makeBytes($data)
        return [MuQrCode]::encodeSegments($seg, $ecl,$mask,$boostEcl)
	}
	...
	static [MuQrCode] encodeSegments([MuQrSegment[]] $segs, [MuEcc] $ecl,[int] $mask=-1, [boolean] $boostEcl=$true) {
        return [MuQrCode]::encodeSegments($segs, $ecl, [QrCodeGlobal]::MIN_VERSION, [QrCodeGlobal]::MAX_VERSION, $mask, $boostEcl)
	}
	...
	static [MuQrCode] encodeSegments([MuQrSegment[]] $segs, [MuEcc] $ecl, [int] $minVersion, [int] $maxVersion, [int] $mask, [boolean] $boostEcl) {
        if (-not $segs)
        {
        ...
        while ($continueLoop) {
		    ...
		    [int] $dataCapacityBits = [MuQrCode]::getNumDataCodewords($tmpVersion, $ecl) * 8 # Number of data bits available
            $dataUsedBits = [MuQrSegment]::getTotalBits($segs, $tmpVersion)
        ...
        if($boostEcl)
        {
			...
			while (-not $nextEcl.isMax())
			{
                ...
                if ($dataUsedBits -le ([MuQrCode]::getNumDataCodewords($tmpVersion, $nextEcl) * 8))
                {
		...
		[int] $dataCapacityBits = [MuQrCode]::getNumDataCodewords($tmpVersion, $ecl) * 8
        ...
        return New-Object 'MuQrCode' $tmpVersion, $ecl, $dataCodewords, $mask
	}
	...
	MuQrCode([int] $ver, [MuEcc] $ecl, [byte[]] $dataCodewords, [int] $msk) {
		...
		$this.version = $ver
        $this.size = ($ver * 2) + 9
        $this.errorCorrectionLevel = New-Object 'MuEcc' $ecl;
    ...
    hidden drawFunctionPatterns()
    {
        ...
        for ([int] $i = 8; $i -lt $this.size; $i++)
        {
            $this.setFunctionModule(0, $i, ($i % 2) -eq 0)
            $this.setFunctionModule($i, 0, ($i % 2) -eq 0)
        }
    ...
    hidden drawFormatBits([int] $msk)
    {
        ...
        {
            $this.setFunctionModule(8, $i, [MuQrCode]::getBit($bits, $i))
        }
        $this.setFunctionModule(8, 7, [MuQrCode]::getBit($bits, 6))
        $this.setFunctionModule(8, 8, [MuQrCode]::getBit($bits, 7))
        $this.setFunctionModule(7, 8, [MuQrCode]::getBit($bits, 8))
        for ([int] $i = 9; $i -lt 15; $i++)
        {
            $this.setFunctionModule(14 - $i, 8, [MuQrCode]::getBit($bits, $i))
        }
        
        # Draw second copy
        for ([int] $i = 0; $i -lt 8; $i++)
        {
            $this.setFunctionModule($this.size - 1 - $i, 8, [MuQrCode]::getBit($bits, $i))
        }
        for ([int] $i = 8; $i -lt 15; $i++)
        {
            $this.setFunctionModule(8, $this.size - 15 + $i, [MuQrCode]::getBit($bits, $i))
        }
    ...
    hidden drawVersion()
    {
        ...
        {
            [boolean] $bit = [MuQrCode]::getBit($bits, $i)
    ...
    hidden [byte[]] addEccAndInterleave([byte[]] $data)
    {
        ...
        if ($data.length -ne [MuQrCode]::getNumDataCodewords($this.version, $this.errorCorrectionLevel))
        {
		...
		[int] $rawCodewords = [Math]::truncate([MuQrCode]::getNumRawDataModules($this.version) / 8)
        [int] $numShortBlocks = $numBlocks - ($rawCodewords % $numBlocks)
        [int] $shortBlockLen = [Math]::truncate($rawCodewords / $numBlocks)
		
        # Split data into blocks and append ECC to each block
        [byte[][]] $blocks = New-Object 'byte[][]' $numBlocks
        [byte[]] $rsDiv = [MuQrCode]::reedSolomonComputeDivisor($blockEccLen)
        [int] $k = 0
        for ([int] $i = 0; $i -lt $numBlocks; $i++)
        {
			...
			[byte[]] $ecc = [MuQrCode]::reedSolomonComputeRemainder($dat, $rsDiv)
	...
	hidden drawCodewords([byte[]] $data) {
		...
		if ($data.length -ne ([Math]::truncate([MuQrCode]::getNumRawDataModules($this.version) / 8)))
		{
		...
		for ([int] $right = $this.size - 1; $right -ge 1; $right -= 2) # Index of right column in each column pair
		{
			...
			{ # Vertical counter
				for ([int] $j = 0; $j -lt 2; $j++)
				{
					...
					{
						$this.modules[$y][$x] = [MuQrCode]::getBit($data[($i -shr 3)], 7 - ($i -band 7))
						$i++
					}
	...
	hidden static [byte[]] reedSolomonComputeDivisor([int] $degree)
	{
		...
		for ([int] $i = 0; $i -lt $degree; $i++)
		{
			...
			{
				$result[$j] = [Convert]::ToByte([MuQrCode]::reedSolomonMultiply($result[$j] -band 0xFF, $root))
			...
			$root = [MuQrCode]::reedSolomonMultiply($root, 0x02)
		}
		return $result
	}
	...
	hidden static [byte[]] reedSolomonComputeRemainder([byte[]] $data, [byte[]] $divisor)
	{
		...
		foreach ($b in $data) # Polynomial division
		{
			...
			{
				$result[$i] = $result[$i] -bxor [MuQrCode]::reedSolomonMultiply($divisor[$i] -band 0xFF, $factor)
			}
		}
		return $result
	}
	...
	static [int] getNumDataCodewords([int] $ver, [MuEcc] $ecl)
	{
		return (([Math]::truncate([MuQrCode]::getNumRawDataModules($ver) / 8)) - ([QrCodeGlobal]::ECC_CODEWORDS_PER_BLOCK[$ecl.getEccOrdinal()][$ver] * [QrCodeGlobal]::NUM_ERROR_CORRECTION_BLOCKS[$ecl.getEccOrdinal()][$ver]))
	}
...
function New-MuQrCode {
	[CmdletBinding()]
	param (
		[Parameter(ParameterSetName="FromString")][string] $text="Sample",
		[Parameter(ParameterSetName="FromSegments")][MuQrSegment[]] $segments,
		[ValidateSet("L","M","Q")][string] $minimumEcc="L",
	...
	[MuEcc] $ecl = New-Object 'MuEcc' $minimumEcc
	
	[MuQrCode] $tmpQr = $null

	if($segments)
	{
		$tmpQr = [MuQrCode]::encodeSegments($segments, $ecl,$forceMask,-not $disalowEccUpgrade)

	}
	else
	{
		$tmpQr = [MuQrCode]::encodeText($text, $ecl,$forceMask,-not $disalowEccUpgrade)
	}
...
function New-QrSegment {
	...
	switch ($Type) {
		"KANJI" {
			[MuQrMode] $qrmd = New-Object 'MuQrMode' "KANJI"
			[int]$kanjiLen = [math]::Truncate($KanjiData.getBitLength()/13)
			return (New-Object 'MuQrSegment' $qrmd, $kanjiLen, $KanjiData)
			}
		"ECI" {
			return [MuQrSegment]::makeEci($EciType)
			}
		"BYTE" {
			return [MuQrSegment]::makeBytes( ([system.Text.Encoding]::UTF8).GetBytes($StringData) )
			}
		"NUMERIC" {
			return [MuQrSegment]::makeNumeric($StringData)
			}
		"ALPHANUMERIC" {
			return [MuQrSegment]::makeAlphanumeric($StringData)
			}
		Default {} # AUTO
	}

	return ([MuQrSegment]::makeSegments($StringData))
}
