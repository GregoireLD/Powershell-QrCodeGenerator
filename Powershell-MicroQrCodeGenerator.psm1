...
class MicroQrMode{
	...
	hidden [int[]] bitsLength;

	# -- Constructor --
		
	hidden Init([int] $qrmode, [int[]] $ccbits, [int[]] $bitsLength)
	{
	...
	hidden Init([int] $qrmode, [int[]] $ccbits)
	{
		$this.modeBits = $qrmode
		$this.numBitsCharCount = $ccbits
		$this.bitsLength = @(0, 1, 2, 3);
	}
	
	MicroQrMode([string] $microqrmode){

		switch ($microqrmode) {
			# "TERMINATOR" { Init(0x00 , @(0, 0, 0, 0) , @(3, 5, 7, 9))}
			"NUMERIC" { Init(0x00 , @(3, 4, 5, 6))}
			"ALPHANUMERIC" { Init(0x01 , @(0, 3, 4, 5))}
			"BYTE" { Init(0x02 , @(0, 0, 4, 5))}
			"KANJI" { Init(0x03 , @(0, 0, 3, 4))}
            Default {
                throw "Invalid MicroQrMode String"
            }
        }
	}
	...
	static [MicroQrMode] TERMINATOR()      {return New-Object 'MicroQrMode' "TERMINATOR"}
	static [MicroQrMode] NUMERIC()      {return New-Object 'MicroQrMode' "NUMERIC"}
	static [MicroQrMode] ALPHANUMERIC()      {return New-Object 'MicroQrMode' "ALPHANUMERIC"}
	static [MicroQrMode] BYTE()      {return New-Object 'MicroQrMode' "BYTE"}
	static [MicroQrMode] KANJI()      {return New-Object 'MicroQrMode' "KANJI"}

	# Returns the bit width of the character count field for a segment in this mode
	# in a Micro QR Code at the given version number.
	[int] numCharCountBits([int] $ver)
	{
		if ($this.numBitsCharCount == $null)
        {
			throw "Character count doesn't apply to this mode"
        }
		return $this.numBitsCharCount[$ver - 1];
	}
}

class MicroQrBitBuffer {
	# ---- Fields ----
	...
	MicroQrBitBuffer([string] $binaryString)
	{
		if (-not($binaryString -match [MicroQrCode]::BINARY_REGEX))
		{
	...
	appendBits([int] $val, [int] $len)
	{
		...
		# 	data.set(bitLength, MicroQrCode.getBit(val, i));
	...
	# @throws IllegalStateException if appending the data
	# would make bitLength exceed Integer.MAX_VALUE
	appendData([MicroQrBitBuffer] $bb)
	{
		if (-not $bb)
        {
	...
	appendBinaryString([string] $binStr)
	{
		...
		if (-not($binStr -match [MicroQrCode]::BINARY_REGEX))
		{
	...
	[MicroQrBitBuffer] clone()
	{
		[MicroQrBitBuffer] $tmpBb = New-Object 'MicroQrBitBuffer'
...
class MicroQrSegment {
	...
	static [MicroQrSegment] makeBytes([byte[]] $data)
	{
		if (-not $data)
        {
            throw "data (Bytes) to forge segment is null"
		}
		[MicroQrBitBuffer] $bb = New-Object 'MicroQrBitBuffer'
		foreach ($b in $data) {
			$bb.appendBits($b -band 0xFF,8)
		}

		return (New-Object 'MicroQrSegment' ([MicroQrMode]::BYTE()),$data.Length,$bb )
	}
	...
	static [MicroQrSegment] makeNumeric([String] $digits)
	{
		if (-not $digits)
        {
            throw "digits (String) to forge segment is null"
		}
		if (-not ($digits -match [MicroQrCode]::NUMERIC_REGEX)){
		...
		return (New-Object 'MicroQrSegment' ([MicroQrMode]::NUMERIC()), $digits.length , $bb)
	}
	...
	static [MicroQrSegment] makeAlphanumeric([String] $text)
	{
		if (-not $text)
        {
		...
		if ( -not ( $text -cmatch [MicroQrCode]::ALPHANUMERIC_REGEX))
		{
		...
		{
			[int] $temp = [MicroQrCode]::ALPHANUMERIC_CHARSET.IndexOf($text[$i]) * 45
			$temp += [MicroQrCode]::ALPHANUMERIC_CHARSET.IndexOf($text[$i+1])
			$bb.appendBits($temp, 11)
		}
		
		if ($i -lt $text.length) # 1 character remaining
		{
			$bb.appendBits([MicroQrCode]::ALPHANUMERIC_CHARSET.IndexOf($text[$i]), 6)
		}
		
		return (New-Object 'MicroQrSegment' ([MicroQrMode]::ALPHANUMERIC()), $text.length , $bb)
	}
	...
	static [MicroQrSegment[]] makeSegments([String] $text)
	{
		if (-not $text)
        {
		...
		[MicroQrSegment[]] $result = @()
		if ($text.equals(""))
		{
			#Leave result empty
		}
		elseif ($text -match [MicroQrCode]::NUMERIC_REGEX )
		{
			$result += ([MicroQrSegment]::makeNumeric($text))
		}
		elseif ($text -cmatch [MicroQrCode]::ALPHANUMERIC_REGEX )
		{
			$result += ([MicroQrSegment]::makeAlphanumeric($text))
		}
		else
		{
			$enc = [system.Text.Encoding]::UTF8
			$bytes = $enc.GetBytes($text)
			$result += ([MicroQrSegment]::makeBytes($bytes))
		}
		return $result
	}
	...
	MicroQrSegment([MicroQrMode] $qrmd, [int] $numCh, [MicroQrBitBuffer] $data)
	{
		if (-not $qrmd)
        {
		...
		if (-not $data)
        {
            throw "data (MicroQrBitBuffer) is null"
		}
	...
	[MicroQrBitBuffer] getData() {
		[MicroQrBitBuffer] $tmpBb = $this.data.clone()
	...
	static [int] getTotalBits([MicroQrSegment[]] $segs, [int] $version)
	{
		if (-not $segs)
		{
			throw "segs (MicroQrSegment[]) is null"
		}
		[long] $result = 0
		foreach ($seg in $segs)
		{
			if (-not $seg)
			{
				throw "inner seg (MicroQrSegment) is null"
			}
...
class MicroQrCode {
    ...
    static [MicroQrCode] encodeText([string] $text, [Ecc] $ecl,[int] $mask=-1, [boolean] $boostEcl=$true)
    {
        if (-not $text)
        {
		...
		[MicroQrSegment[]] $segs = [MicroQrSegment]::makeSegments($text)
        return [MicroQrCode]::encodeSegments($segs, $ecl,$mask,$boostEcl)
	}
    ...
    # largest version QR Code at the ECL, which means it is too long
    static [MicroQrCode] encodeBinary([byte[]] $data, [Ecc] $ecl,[int] $mask=-1, [boolean] $boostEcl=$true)
    {
		if (-not $data)
        {
		...
		[MicroQrSegment] $seg = [MicroQrSegment]::makeBytes($data)
        return [MicroQrCode]::encodeSegments($seg, $ecl,$mask,$boostEcl)
	}
	...
	static [MicroQrCode] encodeSegments([MicroQrSegment[]] $segs, [Ecc] $ecl,[int] $mask=-1, [boolean] $boostEcl=$true) {
        return [MicroQrCode]::encodeSegments($segs, $ecl, [MicroQrCode]::MIN_VERSION, [MicroQrCode]::MAX_VERSION, $mask, $boostEcl)
	}
	...
	static [MicroQrCode] encodeSegments([MicroQrSegment[]] $segs, [Ecc] $ecl, [int] $minVersion, [int] $maxVersion, [int] $mask, [boolean] $boostEcl) {
        if (-not $segs)
        {
            throw "segs (MicroQrSegment[]) in encodeSegments is null"
        }
        if (-not $ecl)
        {
        ...
        if ( -not ((([MicroQrCode]::MIN_VERSION -le $minVersion) -and ($minVersion -le $maxVersion) -and ($maxVersion -le [MicroQrCode]::MAX_VERSION)) -or ($mask -lt -1) -or ($mask -gt 7)))
        {
        ...
        while ($continueLoop) {
		    ...
		    [int] $dataCapacityBits = [MicroQrCode]::getNumDataCodewords($tmpVersion, $ecl) * 8 # Number of data bits available
            $dataUsedBits = [MicroQrSegment]::getTotalBits($segs, $tmpVersion)
        ...
        if($boostEcl)
        {
			...
			while (-not $nextEcl.isMax())
			{
                ...
                if ($dataUsedBits -le ([MicroQrCode]::getNumDataCodewords($tmpVersion, $nextEcl) * 8))
                {
		...
		[int] $dataCapacityBits = [MicroQrCode]::getNumDataCodewords($tmpVersion, $ecl) * 8
        ...
        return New-Object 'MicroQrCode' $tmpVersion, $ecl, $dataCodewords, $mask
	}
	...
	MicroQrCode([int] $ver, [Ecc] $ecl, [byte[]] $dataCodewords, [int] $msk) {
		# Check arguments and initialize fields
        if (($ver -lt [MicroQrCode]::MIN_VERSION) -or ($ver -gt [MicroQrCode]::MAX_VERSION))
        {
			throw "version is out of range. Must range from "+[MicroQrCode]::MIN_VERSION+" to "+[MicroQrCode]::MAX_VERSION
        }
		...
		$this.quietZone=[MicroQrCode]::DEFAULT_QUIET_ZONE
        if (-not $dataCodewords)
        {
            throw "dataCodewords (byte[]) in MicroQrCode is null"
        }
 ...
 # Returns a string of Braille characteres depicting this QR Code, with the specified number
    ...
    hidden drawFormatBits([int] $msk)
    {
        ...
        {
            $this.setFunctionModule(8, $i, [MicroQrCode]::getBit($bits, $i))
        }
        $this.setFunctionModule(8, 7, [MicroQrCode]::getBit($bits, 6))
        $this.setFunctionModule(8, 8, [MicroQrCode]::getBit($bits, 7))
        $this.setFunctionModule(7, 8, [MicroQrCode]::getBit($bits, 8))
        for ([int] $i = 9; $i -lt 15; $i++)
        {
            $this.setFunctionModule(14 - $i, 8, [MicroQrCode]::getBit($bits, $i))
        }
        
        # Draw second copy
        for ([int] $i = 0; $i -lt 8; $i++)
        {
            $this.setFunctionModule($this.size - 1 - $i, 8, [MicroQrCode]::getBit($bits, $i))
        }
        for ([int] $i = 8; $i -lt 15; $i++)
        {
            $this.setFunctionModule(8, $this.size - 15 + $i, [MicroQrCode]::getBit($bits, $i))
        }
    ...
    hidden drawVersion()
    {
        ...
        {
            [boolean] $bit = [MicroQrCode]::getBit($bits, $i)
    ...
    hidden [byte[]] addEccAndInterleave([byte[]] $data)
    {
        ...
        if ($data.length -ne [MicroQrCode]::getNumDataCodewords($this.version, $this.errorCorrectionLevel))
        {
			throw "data.length in addEccAndInterleave is not equal to getNumDataCodewords"
		}
		
		# Calculate parameter numbers
        [int] $numBlocks = [MicroQrCode]::NUM_ERROR_CORRECTION_BLOCKS[$this.errorCorrectionLevel.getEccOrdinal()][$this.version]
        [int] $blockEccLen = [MicroQrCode]::ECC_CODEWORDS_PER_BLOCK[$this.errorCorrectionLevel.getEccOrdinal()][$this.version]
		[int] $rawCodewords = [Math]::truncate([MicroQrCode]::getNumRawDataModules($this.version) / 8)
        [int] $numShortBlocks = $numBlocks - ($rawCodewords % $numBlocks)
        [int] $shortBlockLen = [Math]::truncate($rawCodewords / $numBlocks)
		
        # Split data into blocks and append ECC to each block
        [byte[][]] $blocks = New-Object 'byte[][]' $numBlocks
        [byte[]] $rsDiv = [MicroQrCode]::reedSolomonComputeDivisor($blockEccLen)
        [int] $k = 0
        for ([int] $i = 0; $i -lt $numBlocks; $i++)
        {
			...
			[byte[]] $ecc = [MicroQrCode]::reedSolomonComputeRemainder($dat, $rsDiv)
	...
	hidden drawCodewords([byte[]] $data) {
		...
		if ($data.length -ne ([Math]::truncate([MicroQrCode]::getNumRawDataModules($this.version) / 8)))
		{
		...
		for ([int] $right = $this.size - 1; $right -ge 1; $right -= 2) # Index of right column in each column pair
		{
			...
			{ # Vertical counter
				for ([int] $j = 0; $j -lt 2; $j++)
				{
					...
					{
						$this.modules[$y][$x] = [MicroQrCode]::getBit($data[($i -shr 3)], 7 - ($i -band 7))
						$i++
					}
	...
	hidden [int] getPenaltyScore()
	{
		...
		for([int] $y = 0; $y -lt $this.size; $y++)
		{
			...
			for ([int] $x = 0; $x -lt $this.size; $x++)
			{
				...
				else
				{
					...
					if (-not $runColor)
					{
						$result += $this.finderPenaltyCountPatterns($runHistory) * [MicroQrCode]::PENALTY_N3
					}
					$runColor = $this.modules[$y][$x]
					$runX = 1
				}
			}
			$result += ($this.finderPenaltyTerminateAndCount($runColor, $runX, $runHistory) * [MicroQrCode]::PENALTY_N3)
		}
		# Write-Host "1 - " + $result
		...
		for ([int] $x = 0; $x -lt $this.size; $x++)
		{
			...
			$result += ($this.finderPenaltyTerminateAndCount($runColor, $runY, $runHistory) * [MicroQrCode]::PENALTY_N3)
		}
		# Write-Host "2 - " + $result
		...
		$result += $k * [MicroQrCode]::PENALTY_N4
		# Write-Host "5 - " + $result
	...
	hidden static [int] getNumRawDataModules([int] $ver)
	{
		if (($ver -lt [MicroQrCode]::MIN_VERSION) -or ($ver -gt [MicroQrCode]::MAX_VERSION))
        {
			throw "Version in getNumRawDataModules is not a valid value. It must range from "+[MicroQrCode]::MIN_VERSION+" to "+[MicroQrCode]::MAX_VERSION
		}
	...
	hidden static [byte[]] reedSolomonComputeDivisor([int] $degree)
	{
		...
		for ([int] $i = 0; $i -lt $degree; $i++)
		{
			...
			{
				$result[$j] = [Convert]::ToByte([MicroQrCode]::reedSolomonMultiply($result[$j] -band 0xFF, $root))
			...
			$root = [MicroQrCode]::reedSolomonMultiply($root, 0x02)
		}
		return $result
	}
	...
	hidden static [byte[]] reedSolomonComputeRemainder([byte[]] $data, [byte[]] $divisor)
	{
		...
		foreach ($b in $data) # Polynomial division
		{
			...
			{
				$result[$i] = $result[$i] -bxor [MicroQrCode]::reedSolomonMultiply($divisor[$i] -band 0xFF, $factor)
			}
		}
		return $result
	}
	...
	{
		return (([Math]::truncate([MicroQrCode]::getNumRawDataModules($ver) / 8)) - ([MicroQrCode]::ECC_CODEWORDS_PER_BLOCK[$ecl.getEccOrdinal()][$ver] * [MicroQrCode]::NUM_ERROR_CORRECTION_BLOCKS[$ecl.getEccOrdinal()][$ver]))
	}
...
function New-MicroQrCode {
	...
	.DESCRIPTION
		...
		in all supported formats (Numeric, Alphanumeric, Byte, Kanji).
		You can generate a QRCode either directly from data, or using QRCode segments
		generated with the New-MicroQrSegment Cmdlet.
	...
	.PARAMETER quietZone
		...
		This value is specified in the static variable [MicroQrCode]::DEFAULT_QUIET_ZONE.
	...
	.EXAMPLE
		New-MicroQrCode -text "GLD DEMO" -AsString
	
	.LINK
		https://github.com/GregoireLD/Powershell-QrCodeGenerator
	
	.NOTES
		This Cmdlet can make use of New-MicroQrBitBuffer and New-MicroQrSegment Cmdlets.
    #>
	[CmdletBinding()]
	param (
		[Parameter(ParameterSetName="FromString",ValueFromPipeline=$true)][string] $text="Sample",
		[Parameter(ParameterSetName="FromSegments")][MicroQrSegment[]] $segments,
		[ValidateSet("LOW","MEDIUM","QUARTILE","HIGH")][string] $minimumEcc="LOW",
		[ValidateRange(-2,7)][int] $forceMask=-1,
		[Alias("borderSize")][int] $quietZone=[MicroQrCode]::DEFAULT_QUIET_ZONE_SIZE,
		[switch] $invert,
	...
	if($segments)
	{
		$tmpMicroQr = [MicroQrCode]::encodeSegments($segments, $ecl,$forceMask,-not $disalowEccUpgrade)
	}
	else
	{
		$tmpMicroQr = [MicroQrCode]::encodeText($text, $ecl,$forceMask,-not $disalowEccUpgrade)
	}
	...
	if($quietZone){
		$tmpMicroQr.setQuietZone($quietZone)
	}

	if($asString)
	{
		return ($tmpMicroQr.toString($quietZone))
	}
	
	if($asSvgString)
	{
		return ($tmpMicroQr.toSvgString($quietZone))
	}

 	if($asBrailleString)
	{
		return ($tmpMicroQr.toBrailleString($quietZone))
	}
 	
	return ($tmpMicroQr)
}

function New-MicroQrBitBuffer {
	[CmdletBinding()]
	param (
	...
	return (New-Object 'MicroQrBitBuffer' $binaryString)
}

function New-MicroQrSegment
{
	[CmdletBinding()]
	param (
		[ValidateSet("AUTO","NUMERIC","ALPHANUMERIC","BYTE","KANJI")][string] $Type="AUTO",
		[MicroQrBitBuffer] $KanjiData,
		[string] $StringData,
		[int] $EciType
	)
	...
	switch ($Type) {
		"KANJI" {
			[MicroQrMode] $qrmd = New-Object 'MicroQrMode' "KANJI"
			[int]$kanjiLen = [math]::Truncate($KanjiData.getBitLength()/13)
			return (New-Object 'MicroQrSegment' $qrmd, $kanjiLen, $KanjiData)
			}
		"BYTE" {
			return [MicroQrSegment]::makeBytes( ([system.Text.Encoding]::UTF8).GetBytes($StringData) )
			}
		"NUMERIC" {
			return [MicroQrSegment]::makeNumeric($StringData)
			}
		"ALPHANUMERIC" {
			return [MicroQrSegment]::makeAlphanumeric($StringData)
			}
		Default {} # AUTO
	}

	return ([MicroQrSegment]::makeSegments($StringData))
}
